...
import { EXPATSWAP_MOCK_API_DEFAULT_DELAY } from '@expatswap/lib/mock-api/mock-api.constants';
import { ExpatswapMockApiService } from '@expatswap/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(EXPATSWAP_MOCK_API_DEFAULT_DELAY);
    const expatswapMockApiService = inject(ExpatswapMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = expatswapMockApiService.findHandler(request.method.toUpperCase(), request.url);
